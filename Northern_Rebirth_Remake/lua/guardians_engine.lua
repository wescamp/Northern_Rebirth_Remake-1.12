...
local AH = wesnoth.require "~/add-ons/Northern_Rebirth_Remake/lua/ai_helper.lua"
...
function guardians:coward_execution(id, radius, seek_x, seek_y, avoid_x, avoid_y)
    ...
    if not enemies[1] then
        ai_guardians.stopunit_all(unit)
        return
    end
    ...
    if (mx ~= unit.x or my ~= unit.y) then
        ai_guardians.move(unit, mx, my)
    end

    -- Get unit again, just in case it was killed by a moveto event
    local unit = wesnoth.get_units{ id = id }[1]
    if unit then ai_guardians.stopunit_all(unit) end
end
...
function guardians:return_guardian_execution(id, to_x, to_y)
    ...
    if unit.moves~=0 then
        ai_guardians.move_full(unit, nh[1], nh[2])
    end
end
...
function guardians:stationed_guardian_execution(id, radius, s_x, s_y, g_x, g_y)
    ...
    if not enemies[1] then
        ...
        ai_guardians.stopunit_moves(unit)
        return
    end
    ...
    if (min_dist ~= 9999) then
        ...
        if (best_defense ~= -9999) then
            ...
            ai_guardians.move(unit, attack_loc[1],attack_loc[2])
            -- There should be an ai_guardians.check_attack_action() here in case something weird is 
            -- done in a 'moveto' event.  Not implemented yet in Lua AI.
            ai_guardians.attack(unit, target)
    ...
    local unit = wesnoth.get_units{ id = id }[1]
    if unit then ai_guardians.stopunit_moves(unit) end
